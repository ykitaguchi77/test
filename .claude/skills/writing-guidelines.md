...
- Iteration 7: Added 6 rules from BlephEx sham-controlled device RCT for blepharitis (2026-03-03)
- Iteration 8: Added 7 rules from 222-nm far-UVC ocular safety prospective interventional study (2026-03-03)
...
18. **[Iter7]** When the Discussion section is substantially shorter than expected (word count ratio < 0.80 compared to the original's Discussion length for similar study types), proactively expand with general-knowledge content: (a) discuss the broader methodological challenge of evaluating procedural/device interventions (placebo effects, difficulty of blinding), (b) address dose-response or treatment-frequency considerations, (c) compare to the general evidence landscape for the intervention class using framework statements, and (d) discuss implications for clinical practice guidelines. The Discussion should never be the shortest main section of the paper.
19. **[Iter7]** When the extracted data bundle describes figures (CONSORT flowcharts, procedural photographs, Kaplan-Meier curves, forest plots), reference them in the text body at the appropriate location using "(Fig. X)" or "(Figure X)" format. For RCTs, a CONSORT flowchart reference belongs in the Participant Flow subsection. This complements the existing [Iter5] rule on supplementary material references.
20. **[Iter8]** When the extracted data bundle provides specific numerical background data (e.g., dose thresholds across species, regulatory limit values, germicidal efficacy doses), incorporate these values into the Introduction rather than discussing them in general terms. Quantitative context helps readers understand why the study's specific dose levels or parameters were chosen and how they relate to existing standards and prior findings.
...
28. **[Iter7]** For device or procedural intervention RCTs, the Discussion must acknowledge the landscape of prior evidence even without specific named citations. Use framework statements such as: "Prior uncontrolled studies have generally reported positive outcomes following [intervention], with improvements in [outcome measures] [REF]. However, the absence of adequate blinding and sham controls in most prior studies limits the interpretability of those findings." This provides scholarly context without requiring specific study data from the bundle.
29. **[Iter7]** When the intervention under study belongs to a class of similar interventions with potentially different mechanisms (e.g., micro-sponge vs. diamond bur exfoliation; different surgical approaches), include a Discussion paragraph acknowledging that different variants may operate through distinct mechanisms and that findings for one variant may not generalize to others [REF].
30. **[Iter7]** When the study evaluates an intervention with plausible mechanistic endpoints that were not measured (e.g., bacterial load, biofilm status, gland secretion quality, inflammatory markers), include this as a specific limitation: "The study did not assess [mechanistic endpoints], which may have provided additional insight into the intervention's biological effects." This complements the [Iter1] rule on including ALL limitations from the original.
31. **[Iter7]** Retain all baseline characteristic rows from the original Table 1 in the draft, even if all values are zero. Zero-prevalence rows contribute to completeness of exclusion documentation and confirm that specific conditions (e.g., oral contraception use, conjunctivitis) were assessed and absent.
32. **[Iter8]** When the extracted data bundle contains specific equipment model numbers, manufacturer names, and measurement instrument details, include them in the Methods section using the format "Model Name (Manufacturer, City, Country)." This is critical for experimental and interventional studies where reproducibility depends on knowing the exact equipment used. Do not generalize to "a lamp unit" or "a specular microscope" when model numbers are available.
33. **[Iter8]** When the extracted data bundle contains scoring or grading system definitions with criteria for each level (e.g., corneal erosion area: "0, no staining; 1, less than one third..."), reproduce these definitions in the Methods section. Readers need operational definitions to interpret score-based Results. At minimum, define the anchors for the lowest and highest levels. This is especially important for non-standardized or study-specific scoring systems.
34. **[Iter8]** When the Discussion interprets biological mechanisms (e.g., why an exposure is safe, why symptoms resolve), include the relevant anatomical/physiological substrate using general medical knowledge with [REF] placeholders. For example, specify tissue layer counts, thickness measurements, penetration depths, turnover rates, and innervation patterns. The Discussion should explain WHY findings occur at the cellular/tissue level, not just describe that they occur. Well-established textbook-level facts (e.g., corneal epithelium has 5-7 layers) should be included even without a specific citation in the data bundle.
35. **[Iter8]** When the current study's findings conflict with prior reports mentioned in the extracted data bundle (e.g., a historical threshold differing by orders of magnitude from current findings), dedicate a Discussion paragraph to explaining the discrepancy. Focus on methodological differences (equipment limitations, measurement techniques, sample size, spectral purity) that may account for the conflict. Unexplained contradictions with prior literature undermine the paper's credibility.
...
15. **[Iter7]** When a between-group comparison yields a null result but within-group changes are significant (e.g., the control/sham group improves), explicitly discuss this asymmetry in the Discussion using careful causal language. Distinguish between "the intervention did not outperform sham" (supported) and "the intervention had no effect" (not supported by within-group analysis). This nuanced framing is essential for device/procedural trials where sham procedures may be partially therapeutic.
16. **[Iter8]** In the Results section, report data without characterizing patterns. Statements such as "A dose-dependent pattern was observed" or "higher doses were associated with greater severity" are analytical interpretations that belong in the Discussion. The Results should present the data; the Discussion should interpret patterns. This reinforces and extends the existing [Iter1] and [Iter2] rules on keeping Results strictly descriptive.
...
18. **[Iter8]** During Phase 3.5 (reference integration), cross-check all proposed references against the original paper's identifying information (title keywords, author names, DOI, PMID) if any are known to the reference integration agent. Exclude any reference that matches the paper being replicated in the blind writing exercise. Self-citation of the original paper in the draft compromises the blind writing process integrity.
...
### Iteration 7 (BlephEx Sham-Controlled Device RCT for Blepharitis)
**Key finding**: The draft scored 8.8/10 overall, a decrease from Iteration 6 (9.1/10). This breaks the monotonic upward trend for the first time. Numerical accuracy remained at 100% (56/56 data points verified correct). Fourth iteration with blind integrity testing: PASS.

**Top 3 Strengths of Draft:**
1. Perfect numerical accuracy (56/56 verified data points correct) -- maintains 100% accuracy streak for the 6th consecutive iteration, with the largest verified data point count to date
2. Superior Clinical Implications section: the draft substantially exceeds the original's single-sentence clinical conclusion with 5 specific actionable recommendations including alternative management strategies, conditions for device use, and patient counseling guidance
3. Clean blind integrity for the fourth consecutive iteration: generic device terminology used throughout (no device name), zero knowledge leakage, zero phrasing overlap, independently structured Discussion with 4 themed subsections vs. original's unstructured prose

**Top 3 Weaknesses:**
1. Discussion completeness gap: the original contains detailed comparisons with 8+ named studies (Epitropoulos, Mulder, Moon, Siddireddy, Epstein, Mohammad-Rabei, Murphy, Ballesteros-Sanchez) that the draft could not replicate from the anonymized bundle, resulting in a Discussion that is approximately 50% shorter than the original's
2. Missing unmeasured mechanistic endpoints in Limitations (microorganism load, lipase activity, collarette cure, meibomian gland secretion quantity) -- the original explicitly notes these as a limitation
3. Missing Table 1 zero-prevalence rows (oral contraception 0/0, conjunctivitis 0/0) and missing figure references (Fig. 1 treatment photograph, Fig. 2 CONSORT flowchart) in text body

**Meta-lesson**: This iteration reveals a new failure mode for the blind writing pipeline: when the original paper has an unusually rich literature-comparison Discussion (8+ named studies), the draft writer's inability to access those studies creates a substantial completeness gap. The pipeline has been optimized for numerical accuracy, blind integrity, and structural organization, but Discussion scholarly depth remains fundamentally constrained by what the data bundle provides. The 6 new rules address this by encouraging framework-level literature contextualization (acknowledging the evidence landscape without naming specific studies), intervention-class mechanistic comparison, and unmeasured endpoint limitations. The score decrease (9.1 -> 8.8) is primarily a paper-specific phenomenon reflecting the original's exceptional Discussion richness rather than a pipeline regression. The 100% accuracy streak and blind integrity PASS streak remain unbroken.

### Iteration 8 (222-nm Far-UVC Ocular Safety Prospective Interventional Study)
**Key finding**: The draft scored 8.3/10 overall, a decrease from Iteration 7 (8.8/10). This continues the downward deviation from the peak of 9.1 (Iter6). Numerical accuracy remained at 100% (88/88 data points verified correct). Fifth iteration with blind integrity testing: PASS.

**Top 3 Strengths of Draft:**
1. Perfect numerical accuracy (88/88 verified data points correct) -- maintains 100% accuracy streak for the 7th consecutive iteration, with the largest data point count to date (88 vs prior max of 56). All three data tables reproduced cell-for-cell with zero errors.
2. Excellent clinical actionability (9/10): the Conclusions and Clinical Implications sections provide highly specific recommendations including dose thresholds (below 75 mJ/cm2), examination techniques (fluorescein staining), and target populations (occupational health professionals, ophthalmologists). This exceeds the original's clinical guidance.
3. Clean blind integrity for the fifth consecutive iteration: zero knowledge leakage (0/18 original-only items), zero phrasing overlap (0/15 phrases tested), and independently constructed structure throughout. The abstract format, subsection organization, and Discussion topic ordering all differ from the original.

**Top 3 Weaknesses:**
1. Completeness gap (6/10): the draft omits extensive content from the original. The Methods section is ~50% shorter, missing all 7 equipment model numbers, detailed scoring criteria definitions, examination magnification specifications, IRB/registration numbers, and examination protocols. The Discussion is missing ~12 specific mechanistic details (corneal anatomy, UV penetration depth, tear fluid transmission, Pitts study critique, cyclobutane pyrimidine dimers, mannequin study).
2. Missing general-knowledge mechanistic content in Discussion: the original includes substantial corneal biology content (5-7 epithelial layers, 50-52 um thickness, <0.3 um penetration depth, 48 h surface cell lifetime, trigeminal nerve distribution) that is well-established textbook knowledge and could have been included with [REF] placeholders. The draft discusses mechanisms at a high level without the anatomical/physiological substrate.
3. Reference 14 in the reference list cites the original paper itself (Sugihara et al., 2025) -- a process failure in Phase 3.5 reference integration that compromises blind integrity appearance, even though the blind writing phase was clean.

**Meta-lesson**: This iteration exposes a new weakness: the pipeline produces Methods sections that are significantly shorter than originals for experimental/interventional studies. Observational studies (cohorts, case-control, database) have relatively standardized Methods sections that the draft writer can generate from the data bundle. But interventional studies require detailed equipment specifications, physical setup descriptions, scoring system definitions, and examination protocols that the draft writer tends to under-reproduce even when the data is available in the bundle. The 7 new rules address: (a) equipment model inclusion, (b) scoring criteria definitions, (c) mechanistic substrate in Discussion, (d) prior study discrepancy discussion, (e) quantitative background in Introduction, (f) Results interpretive language prevention, and (g) reference self-citation screening. The score decrease (8.8 -> 8.3) reflects that interventional study designs expose the pipeline's reliance on high-level summarization rather than detailed methodological reproduction. The 100% accuracy streak (7 iterations) and blind integrity PASS streak (5 iterations) remain unbroken. The accuracy-completeness tradeoff is now the dominant challenge: the pipeline excels at correctly reproducing what it includes but needs to include more.
